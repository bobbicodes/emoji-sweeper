(ns ^:figwheel-hooks emoji-sweeper.core
  (:require
   [goog.dom :as gdom]
...
(def percent-mines (reagent/cursor app-state [:percent-mines]))
(def width (reagent/cursor app-state [:width]))
(def height (reagent/cursor app-state [:height]))
...
(defn mines-input []
  (number-input " % mines: " @percent-mines
                (fn [e] (reset! percent-mines
                                (js/parseInt (-> e .-target .-value)))
                  (new-board!))))

(defn set-mines [[x y]]
  (set (take (* (/ @percent-mines 100)
                (* @height @width))
